...
class NI9234(NIDAQDevice):
    """NI 9234: 4-Kanal IEPE-Eingang für Beschleunigungssensoren/Mikrofone.

    Erwartet, dass alle übergebenen Kanäle `signal_type ==
    SignalType.IEPE_ACCELERATION` verwenden und eine
    `sensitivity_mv_per_unit` (Sensorempfindlichkeit in mV/g) angeben, da
    diese für die korrekte hardwareseitige Skalierung zwingend erforderlich ist.
    """
        ...
        for channel in channels:
            if channel.signal_type != SignalType.IEPE_ACCELERATION:
                raise AcquisitionError(
                    f"NI9234 unterstützt nur SignalType.IEPE_ACCELERATION, "
                    f"Kanal '{channel.display_name}' hat {channel.signal_type}."
                )
            if channel.sensitivity_mv_per_unit is None:
                raise AcquisitionError(
